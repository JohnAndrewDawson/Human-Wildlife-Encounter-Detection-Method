...
sql3 = """
select a_.id_encounter, st_union(b_.ppa) as geom
from exp_encounter_event_default	as a_
left join (select 
		   distinct 
		   		id_point, 
		   		ppa 
		   from exp_ppa_default)	as b_
	on a_.id_point = b_.id_point
group by a_.id_encounter
"""

sql4 = """
select a_.id_encounter, st_buffer(ST_Simplify(st_collect(b_.geom_line),5),20) as geom
from exp_encounter_event_default 	as a_
left join (select 
		   distinct 
		   		id_point, 
		   		geom_line 
		   from exp_hda_default)	as b_
	on a_.id_point_2 = b_.id_point
group by a_.id_encounter
"""
...
from exp_encounter_default	as a_
